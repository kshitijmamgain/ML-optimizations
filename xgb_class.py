...
class XGB_Higgs():
    ...
    runs an XGBoost classifier with cross-validation, and attempts to optimize
    the hyperparameters using three different methods: HyperOpt, Optuna and
    RandomSearch.
    """
    ...
    def split_data(self):
        ...
        self.y_train, self.y_test = train_test_split(x, y, test_size=0.3)
    ...
    def cross_validation(self, space):
        ...
        ----------
        space: dict
            A dictionary containing the parameters to use when training
            the model
    ...
    def hyperopt_tuning(self):
        ...
        """
        Applies the HyperOpt algorithm to tune the hyperparameters of the
        XGBoost model and stores the results in a dataframe

        Returns
        -------
        best: dict
            A dictionary containing the set of parameters that resulted in the
            best value of the loss function using the HyperOpt optimization
            algorithm
        """
        print("Starting HyperOpt hyperparameter tuning...")
        self.hyperopt_results = pd.DataFrame(columns=["params",
                                                      "loss",
                                                      "variance",
                                                      "time"]
                                             )
        ...
        def hyperopt_params():
            ...
            bootstrap_type = [{"bootstrap_type": "Bayesian",
                               "bagging_temperature": hp.loguniform(
                                  "bagging_temperature", np.log(1), np.log(50))
                               },
        ...
        def hyperopt_objective(space):
            ...
            ----------
            space: dict
                A dictionary of parameters to use for each trial of the
                HyperOpt optimization algorithm

            Returns
            -------
            result_dict: dict
                A dictionary containing the loss for the current set of
                parameters, the status of the current trial and a dictionary of
            ...
            self.hyperopt_results = self.hyperopt_results.append(
        ...
        best = self.hyperopt_results[["params", "loss"]].sort_values(
    ...
    def optuna_tuning(self):
        ...
        best: dict
            A dictionary containing the set of parameters that resulted in the
            best value of the loss function using the Optuna optimization
            algorithm
        """
        print("Starting Optuna hyperparameter tuning...")
        self.optuna_results = pd.DataFrame(columns=['params',
                                                    'loss',
                                                    'variance',
                                                    'time']
                                           )
        ...
        def optuna_objective(trial):
            ...
            """
            Defines the objective function to be optimized by the Optuna
            algorithm
            ...
            -------
            cv_score: float
                The value of the loss function cross-validated over n-folds for
                the current set of hyperparameters for the trial
            """
            ...
            if space["booster"] == "dart":
                ...
                space["rate_drop"] = trial.suggest_uniform(
                                        "rate_drop", 0, 1
                                        )
                space["skip_drop"] = trial.suggest_uniform(
                                        "skip_drop", 0, 1
                                        )

            results = self.cross_validation(space)

            self.optuna_results = self.optuna_results.append(
                                            {'params': results['params'],
                                             'loss': results['loss'],
                                             'variance': results['variance'],
                                             'time': results['time']},
    ...
    def random_search_tuning(self):
        ...
        best: dict
            A dictionary containing the set of parameters that resulted in the
            best value of the loss function using the Random Search
            optimization algorithm
        """
        print("Starting Random Search hyperparameter tuning...")
        self.random_search_results = pd.DataFrame(columns=['params',
                                                           'loss',
                                                           'variance',
                                                           'time']
                                                  )
        ...
        def random_params():
            ...
            """
            Defines the hyperparameter search space for the Random Search
            algorithm
        ...
        def random_objective(space):
            ...
            ----------
            space: dict
                A dictionary of parameters to use for each trial of the Random
            ...
            self.random_search_results = self.random_search_results.append(
                                            {'params': results['params'],
                                             'loss': results['loss'],
                                             'variance': results['variance'],
                                             'time': results['time']},
        ...
        best = self.random_search_results[['params', 'loss']].sort_values(
    ...
    def confusion_matrix(self):
        ...
        """
        for i in self.output:
            cm = sklearn.metrics.confusion_matrix(self.y_test,
                                                  np.round(self.output[i]
                                                           ['prediction']))
            print(i)
            plt.figure()
    ...
    def classification_report(self):
        ...
        for i in self.output:
            ...
            print(sklearn.metrics.classification_report(
                                        self.y_test,
                                        np.round(self.output[i]['prediction'])
                                                        ))
    ...
    def pr_curve(self):
        ...
        for i in self.output:
            ...
            precision, recall, thresholds = p, r, t
            ...
            plt.title(
             i +
             '- Precision-Recall Curve: AP={0:.2f}'.format(average_precision)
                )
